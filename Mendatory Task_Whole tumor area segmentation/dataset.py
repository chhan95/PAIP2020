...
def prepare_PAIP2020_data():
    ...
    data_root_dir = '/media/data1/han/PAIP2020_DATA/training_set'
    train_set = []
    valid_set = []
    test_set=[]

    for i in range(1,48):
        train_set += load_data_info('%s/benign/%s/*.png' % (data_root_dir, i),
                                    parse_label=False, label_value=0)
        train_set += load_data_info('%s/tumor/%s/*.png' % (data_root_dir, i),
                                    parse_label=False, label_value=1)

    return train_set[:], train_set[-200:]
